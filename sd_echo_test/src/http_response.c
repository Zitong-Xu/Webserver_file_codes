...
int do_http_post(int sd, char *req, int rlen)
{
	...
	if (is_cmd_file(req))
  {
	 ...
	 {
		xil_printf("%c",req[i]);
		if(req[i]=='$') {rstart=i;xil_printf("rstart=%d",rstart);}
		if(req[i]=='#') {rend=i;xil_printf("rend=%d",rend);}
     }

	 int read_len = rend-rstart-1;
	 int j=0;
	 xil_printf("\r\nstart getting short reads:\r\n");
	 xil_printf("read_len=%d\n",read_len);

	for(i=rstart;i<rend;i++)
	{
		reads[i-rstart]=req[i+1];
		//xil_printf("%c",reads[i-rstart]);
	}

	xil_printf("\n----get reads from http POST request----\n");
	...
	for(i=0;i<=read_len;i++)
    {
		...
		{
		    mapper[j][i]=reads[i];
		    xil_printf("%c",mapper[j][i]);
		}
		else if((reads[i]==13) | (reads[i]=='#'))
		{
			j=j+1;
			xil_printf("--get read No.%d--",j);
		    xil_printf("\n");
		}
	}
